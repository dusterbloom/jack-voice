...
use super::{RealtimeVad, RealtimeStt, RealtimeTts, LlmClient, LlmConfig, ChatMessage, TranscriptionEvent};
...
impl VoicePipeline {
    pub fn new(config: PipelineConfig, event_sender: mpsc::Sender<PipelineEvent>) -> Result<Self> {
        let vad = if config.vad_enabled {
            Some(RealtimeVad::with_config(config.vad_threshold, config.vad_silence_ms)?)
        } else {
            None
        };
    ...
    pub async fn process_audio(&self, audio_data: &[u8]) -> Result<()> {
        ...
        if let Some(ref mut vad) = *vad_guard {
            ...
            if result.is_speech {
                self.event_sender.send(PipelineEvent::SpeechStarted {
                    audio_start_ms: 0,
                }).await.ok();
            }
        }
    ...
    pub async fn commit_audio(&self) -> Result<String> {
        ...
        if let Some(ref mut stt) = *stt_guard {
            ...
            if result.is_partial || result.is_final {
                self.event_sender.send(PipelineEvent::Transcription(
                    if result.is_final {
            ...
            self.event_sender.send(PipelineEvent::SpeechStopped {
    ...
    pub async fn generate_response(&self, context: Vec<ChatMessage>) -> Result<()> {
        ...
        let llm = llm_guard.as_ref().ok_or_else(|| anyhow::anyhow!("LLM not initialized"))?;
...
mod tests {
    ...
    fn test_pipeline_config_custom() {
        let config = PipelineConfig {
            ...
            tts_voice: Some("af_sarah".to_string()),
            turn_detection: false,
        };
        
        assert!(!config.vad_enabled);
        assert_eq!(config.llm_config.model, "custom-model");
        assert_eq!(config.tts_voice, Some("af_sarah".to_string()));
    }
}
