...
async fn test_protocol_message_parsing() {
    ...
    match event {
        ClientEvent::ResponseCreate(create) => {
            assert!(create.response.is_some());
        }
...
async fn test_server_event_serialization() {
    ...
    let event = ServerEvent::InputAudioTranscriptionCompleted(crate::protocol::InputAudioTranscriptionCompleted {
...
async fn test_audio_buffer_integration() {
    use crate::audio::{AudioBuffer, pcm16_to_f32};
...
async fn test_session_manager_integration() {
    ...
    // Create session
    let response = manager.create_session(CreateSessionRequest {
    ...
    // Update session
    let updated = manager.update_session("test_session", UpdateSessionRequest {
