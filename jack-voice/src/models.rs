...
pub async fn ensure_qwen_model(
    ...
    tokio::task::spawn_blocking(move || -> Result<(), ModelError> {
        ...
        log::info!(
            "[MODELS] Downloading Qwen model via hf-hub: {}",
            model_id_owned
        );
        ...
        fs::create_dir_all(&target_dir).map_err(|e| ModelError::IoError(e.to_string()))?;
        ...
        fn copy_file(src: &std::path::Path, dst: &std::path::Path) -> Result<(), ModelError> {
            fs::copy(src, dst).map_err(|e| {
                ModelError::IoError(format!("Failed to copy {:?} to {:?}: {}", src, dst, e))
            })?;
            Ok(())
        }
        ...
        copy_file(
            &paths.decoder_weights,
            &target_dir.join("speech_tokenizer/model.safetensors"),
        )?;
...
pub fn qwen_onnx_model_ready(use_int8: bool) -> bool {
    ...
    model_dir
        .join(format!("talker_prefill{}.onnx", suffix))
        .exists()
        && model_dir
            .join(format!("talker_decode{}.onnx", suffix))
            .exists()
...
pub async fn ensure_qwen_onnx_model(
    ...
    if qwen_onnx_model_ready(use_int8) {
        log::info!(
            "[MODELS] Qwen ONNX {} model already ready",
    ...
    let (model_id, size_mb, model_name) = if use_int8 {
        (
            QWEN_ONNX_INT8_MODEL_ID,
            QWEN_ONNX_INT8_SIZE_MB,
            "Qwen ONNX INT8 (0.6B)",
        )
    } else {
        (
            QWEN_ONNX_FP16_MODEL_ID,
            QWEN_ONNX_FP16_SIZE_MB,
            "Qwen ONNX FP16 (0.6B)",
        )
    };
    ...
    tokio::task::spawn_blocking(move || -> Result<(), ModelError> {
        log::info!(
            "[MODELS] Downloading Qwen ONNX model from HuggingFace: {}",
            model_id_owned
        );
        ...
        for file in files_to_download {
            ...
            let response = reqwest::blocking::get(&url).map_err(|e| {
                ModelError::DownloadError(format!("Failed to download {}: {}", file, e))
            })?;

            if !response.status().is_success() {
                // Some files may not exist (e.g., .data files)
                log::warn!(
                    "[MODELS] Could not download {} (status: {})",
                    file,
                    response.status()
                );
                continue;
            ...
            let bytes = response.bytes().map_err(|e| {
                ModelError::DownloadError(format!("Failed to read {}: {}", file, e))
            })?;
...
pub fn voice_library_dir() -> PathBuf {
    get_models_dir().expect("models dir").join("voices")
}
...
mod tests {
    ...
    fn qwen_model_dir_lite_path() {
        ...
        assert!(
            ...
            "Path should contain 'qwen': {}",
            path_str
        );
        assert!(
            path_str.contains("lite"),
            "Path should contain 'lite': {}",
            path_str
        );
    }
    ...
    fn qwen_model_dir_large_path() {
        ...
        assert!(
            path_str.contains("large"),
            "Path should contain 'large': {}",
            path_str
        );
    }
    ...
    fn voice_library_dir_contains_voices_segment() {
        let dir = voice_library_dir();
        assert!(
            dir.to_string_lossy().contains("voices"),
            "Path should contain 'voices': {:?}",
            dir
        );
    }
    ...
    fn voice_library_path_includes_name_and_extension() {
        ...
        assert!(
            path.to_string_lossy().ends_with("my_voice.bin"),
            "Path should end with name.bin: {:?}",
            path
        );
    }
    ...
    fn voice_library_save_load_roundtrip_preserves_data() {
        ...
        assert_eq!(
            embedding, loaded,
            "Loaded embedding should match saved data exactly"
        );
    }
    ...
    fn voice_library_list_returns_exactly_saved_voices() {
        ...
        assert_eq!(
            voices.len(),
            2,
            "Should have exactly 2 saved voices, got: {:?}",
            voices
        );
    ...
    fn voice_library_delete_removes_and_prevents_load() {
        ...
        assert!(
            voice_embedding_exists("to_delete"),
            "Voice should exist after save"
        );

        delete_voice_embedding("to_delete").expect("delete should work");
        assert!(
            !voice_embedding_exists("to_delete"),
            "Voice should not exist after delete"
        );
        assert!(
            load_voice_embedding("to_delete").is_err(),
            "Loading deleted voice should fail"
        );

        let voices = list_saved_voices().expect("list should work");
        assert!(
            voices.is_empty(),
            "List should be empty after deleting the only voice"
        );
    }
